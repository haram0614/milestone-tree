...
addLayer("ap", {
	...
	challenges: {
		...
		completionsAfter120(){
				...
				return p.log10().div(c).log(a).pow(new Decimal(1).div(b).mul(buyableEffect('ap', 12))).toNumber();
		},
		...
		completionsAfter120(){
			if(player.m.effective.gte(245)){
				return player.points.add(10).log10().div(500).mul(buyableEffect('ap', 12)).log(1.035).toNumber();
			}
